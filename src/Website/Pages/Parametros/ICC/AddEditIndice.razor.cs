...
namespace Website.Pages.Parametros.ICC
{
    ...
    {
        [Parameter] public int? IdIcc { get; set; } = null!;
        [Inject] private DialogService DS { get; set; } = null!;
        [Inject] private NotificationService notificationService { get; set; } = null!;
        [Inject] NavigationManager navigationManager { get; set; } = null!;
        [Inject] private DialogService dialogService { get; set; } = null!;

        [Inject] private IIndiceConstruccionBL _indicesBL { get; set; } = null!;
        private IndiceConstruccionModel Model { get; set; } = new();
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            {
                cfg.CreateMap<IndiceConstruccionModel, IndiceCostoConstruccionDTO>().ReverseMap();
            });
            ...
            if (IdIcc.HasValue)
            {
                var dto = await _indicesBL.GetIndiceAsync(IdIcc.Value);
                Model =_mapper.Map<IndiceConstruccionModel>(dto);
            }
        ...
        protected async Task OnClickAceptar(EditContext Ed)
        {
            ...
            if (Ed != null && Ed.Validate())
            {
                ...
                try
                {
                    var dto = _mapper.Map<IndiceConstruccionModel, IndiceCostoConstruccionDTO>(Model);

                    if (Model.IdIcc.HasValue)
