...
namespace Website.Pages.Parametros.Especialidades.Tareas
{
    public partial class Index: ComponentBase
    {
        ...
        private List<EspecialidadTareaDTO> lstTareas { get; set; } = new();
        private RadzenDataGrid<EspecialidadTareaDTO>? grdTareas = null!;
        ...
        {
            lstTareas = await especialidadesBL.GetEspecialidadesTareasAsync();
            grdTareas?.Reload();
        }
        protected async Task AgregarClick()
        {
            var respuesta = await DS.OpenAsync<AddOrEdit>("Agregar Tarea", null,
        ...
        private async Task EditarClick(EspecialidadTareaDTO row)
        {

            var dto = await especialidadesBL.GetEspecialidadEquipoAsync(row.IdSeccion);
            var respuesta = await DS.OpenAsync<AddOrEdit>("Editar Tarea",
                                        new Dictionary<string, object>() { { "IdTarea", row.IdTarea } },
                                        new DialogOptions() { Width = "60%", Height = "auto", Resizable = true });
        ...
        protected async Task EliminarClick(EspecialidadTareaDTO row)
        {
            try
            {
                ...
                if (respuesta ?? false)
                {
                    await especialidadesBL.EliminarEspecialidadTareaAsync(row.IdTarea);
                    await ReloadGrillaAsync();
                }
            }
            catch (Exception ex)
            {
                if (ex.InnerException is SqlException)
                {
                    ...
                    {
                        row.Baja = true;
                        await especialidadesBL.ActualizarEspecialidadTareaAsync(row);
                        await ReloadGrillaAsync();
                    }
                }
                else
